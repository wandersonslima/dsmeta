...
public class SmsService {
	...
	public void sendSms(Long vendaId) {
		...
		Vendas venda = vendaRepository.findById(vendaId).get();
		
		String date = venda.getDate().getMonthValue() + "/" + venda.getDate().getYear();
		
		String msg = "Vendedor " + venda.getSellerName() + " foi destaque em " + date + " com um total de R$ " + String.format("%.2f", venda.getAmount());
